...
import type { Workspace, WorkspaceMember } from '@shared/schemas/workspace.schema'
...
export default function SettingsPage() {
    ...
    const handleInvite = async (e: React.FormEvent) => {
        ...
        setIsInviting(true)
        try {
            const res = await workspaceApi.inviteMember({ email: inviteEmail, fullName: inviteName })
    ...
    const handleUpdateWorkspace = async (updates: Partial<Pick<Workspace, 'name' | 'logoUrl'>>) => {
        try {
                ...
                <section className="mb-10">
                    ...
                    <div className="space-y-6">
                        ...
                        <div className="bg-[#0d0d0d] border border-primary/15 rounded-xl p-6">
                            <h3 className="text-sm font-semibold text-white mb-4">Workspace Profile</h3>
                            <div className="flex gap-6 items-center">
                                ...
                                <div className="flex-1 space-y-4">
                                    <div>
                                        <label className="block text-[10px] font-bold text-primary uppercase tracking-widest mb-1.5 ml-1">Workspace Name</label>
                                        <input
                                            type="text"
                                            defaultValue={workspace?.name}
                                            onBlur={(e) => handleUpdateWorkspace({ name: e.target.value })}
                                    ...
                                    <div>
                                        <label className="block text-[10px] font-bold text-primary uppercase tracking-widest mb-1.5 ml-1">Logo URL</label>
                                        <input
                                            type="text"
                                            defaultValue={workspace?.logoUrl}
                                            onBlur={(e) => handleUpdateWorkspace({ logoUrl: e.target.value })}
                                            className="w-full bg-[#1e1a14]/50 border border-primary/20 rounded-lg text-white text-sm py-2 px-4 focus:ring-primary focus:border-primary"
                                        />
                                    </div>
                                </div>
                ...
                <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm">
                    <div className="bg-[#0d0d0d] border border-primary/20 w-full max-w-md rounded-2xl shadow-2xl p-8 text-center">
                        ...
                        <div className="bg-primary/5 border border-primary/20 rounded-xl p-4 mb-6">
                            <p className="text-sm text-slate-300 font-mono break-all select-all">{tempPasswordMsg}</p>
                        </div>
                        <p className="text-xs text-slate-500 mb-6">Copy the temporary password above and send it to the recruiter so they can log in.</p>
                        <button
