...
const STATUS_CONFIG: Record<string, { label: string; color: string; bg: string; icon: string }> = {
    'UPCOMING': { label: 'Upcoming', color: 'text-blue-400', bg: 'bg-blue-500/10 border-blue-500/20', icon: 'schedule' },
    'ACTIVE': { label: 'Active', color: 'text-primary', bg: 'bg-primary/10 border-primary/20', icon: 'play_circle' },
...
export default function InterviewsListPage() {
    ...
    const handleStartInterview = useCallback((_interview: InterviewListItem) => {
        navigate('/hardware')
    }, [navigate])

    const handleViewResults = useCallback((interview: InterviewListItem) => {
    ...
    const handleCreateInterview = async () => {
        ...
        try {
            ...
            setNewInterview({ title: '', jobTitle: '', instructions: '', questionCount: 5, personaId: '', durationMinutes: 45, toolsEnabled: { codeEditor: true, whiteboard: false, notes: true } })
            setDraftedQuestions('')
            setCreateStep('info')
            fetchInterviews()
        } catch (error) {
                    ...
                    <div className="bg-[#0d0d0d] border border-primary/20 w-full max-w-2xl rounded-2xl shadow-2xl overflow-hidden flex flex-col max-h-[90vh]">
                            ...
                            {createStep === 'info' ? (
                                <>
                                    <div className="space-y-4 mb-6">
                                        <div className="grid grid-cols-2 gap-6">
                                            ...
                                            <div className="space-y-2">
                                                ...
                                                <input
                                                    ...
                                                    onChange={e => setNewInterview({ ...newInterview, durationMinutes: parseInt(e.target.value) || 45 })}
                                                    className="w-full bg-[#1e1a14]/50 border border-primary/20 rounded-lg text-white text-sm py-3 px-4 focus:ring-primary focus:border-primary"
                                                />
                                            </div>
                                        </div>
                                        ...
                                        <div className="space-y-2">
                                            ...
                                            <select
                                                ...
                                                onChange={e => setNewInterview({ ...newInterview, personaId: e.target.value })}
                                            ...
                                            >
                                                <option value="">Default AI (Atlas-7)</option>
                                                {personas.map(p => (
                                        ...
                                        <div className="space-y-2">
                                            <label className="text-[10px] font-bold text-primary uppercase tracking-widest ml-1">Tools Enabled</label>
                                                ...
                                                {(['codeEditor', 'whiteboard', 'notes'] as const).map(tool => (
                                                    <button
                                                        ...
                                                        onClick={() => setNewInterview({
                                                            ...newInterview,
                                                            toolsEnabled: { ...newInterview.toolsEnabled, [tool]: !newInterview.toolsEnabled[tool] }
                                                        })}
                                    ...
                                    <div className="p-6 bg-primary/5 border border-primary/10 rounded-xl space-y-6">
                                        <div className="flex items-center gap-3 border-b border-primary/10 pb-4">
                                            ...
                                            <div>
                                                <h4 className="text-sm font-bold text-white">AI Question Generator</h4>
                                                <p className="text-[10px] text-slate-500 uppercase tracking-widest">Powered by Gemini 3.0 Flash</p>
                                            </div>
                                        </div>
                                            ...
                                            <div className="grid grid-cols-2 gap-4">
                                                ...
                                                <div className="space-y-2">
                                                    ...
                                                    <input
                                                        ...
                                                        onChange={e => setNewInterview({ ...newInterview, questionCount: parseInt(e.target.value) || 5 })}
                                                        className="w-full bg-[#0d0d0d] border border-primary/10 rounded-lg text-white text-sm py-2 px-4"
                                                    />
                                                </div>
                                            </div>
                                ...
                                <div className="space-y-6">
                                    <div className="flex items-center justify-between border-b border-primary/10 pb-4">
                                        ...
                                        <button onClick={() => setCreateStep('info')} className="text-[10px] font-bold text-primary uppercase hover:underline">Edit Info</button>
                                    </div>
                                    <p className="text-xs text-slate-500">Edit the generated questions below, or write your own from scratch.</p>
                                    <textarea
                        ...
                        <div className="p-6 border-t border-primary/10 bg-primary/5 flex gap-4">
                            {createStep === 'info' ? (
                                ...
                                >
                                    {isGenerating ? <div className="size-4 border-2 border-black/30 border-t-black rounded-full animate-spin" /> : <span className="material-symbols-outlined">auto_awesome</span>}
                                    Generate Questions
                                </button>
                            ) : (
                                <>
                                    ...
                                    >
                                        <span className={`material-symbols-outlined ${isGenerating ? 'animate-spin' : ''}`}>refresh</span>
                                    </button>
                                    <button
                                        ...
                                        className="flex-1 py-4 bg-primary text-black font-bold uppercase tracking-[0.2em] rounded-lg hover:brightness-110 disabled:opacity-50 transition-all flex items-center justify-center gap-2 shadow-xl shadow-primary/10"
                                    >
                                        {isCreating && <div className="size-4 border-2 border-black/30 border-t-black rounded-full animate-spin" />}
                                        Finalize & Create
                                    </button>
                                </>
                            )}
                ...
                <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm">
                    <div className="bg-[#0d0d0d] border border-primary/20 w-full max-w-md rounded-2xl shadow-2xl p-8 text-center">
                        ...
                        <div className="bg-primary/5 border border-primary/20 rounded-xl p-6 mb-6">
                            <p className="text-4xl font-bold text-primary font-mono tracking-[0.5em] select-all">{createdAccessCode}</p>
                        </div>
                        <button
