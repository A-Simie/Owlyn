...
const MOCK_TRANSCRIPT = [
  {
    ...
    time: "00:12:45",
    speaker: "ai" as const,
    text: '"Can you describe a time when you had to resolve a production-level race condition in a distributed environment?"',
  },
  {
    id: "2",
    time: "00:13:12",
    speaker: "candidate" as const,
    text: '"We were seeing intermittent failures in our transaction processing layer. After profiling the actor model implementation, I identified that the lock acquisition was biased. I refactored the scheduling logic to use a fair-wait queue, which reduced latent collisions by 40%..."',
    isKey: true,
    aiNote:
      "Candidate demonstrates specific technical depth and metrics-driven results.",
  },
  {
    id: "3",
    time: "00:15:20",
    speaker: "candidate" as const,
    text: '"One of the biggest lessons I learned from that was the importance of observability before attempting a fix. If we hadn\'t tagged the trace IDs properly, we would have been hunting shadows."',
  },
];
...
export default function AnalysisPage() {
  ...
  const handleFinalize = async () => {
    ...
    try {
      await interviewsApi.finalizeReport(sessionId, { decision, notes });
          ...
          <div className="flex gap-6 items-center">
            ...
            <div>
              ...
              <p className="text-primary font-medium tracking-wide flex items-center gap-2">
                ...
                <span className="size-1 bg-primary/40 rounded-full" /> Interview
                Date: Oct 24, 2023
              </p>
            </div>
          </div>
            ...
            <div className="surface-card p-6 rounded flex-1">
              ...
              <div className="aspect-square w-full relative flex items-center justify-center">
                ...
                >
                  ...
                  />
                  <polygon
                    points="100,26 172,100 100,164 44,100"
                    fill="rgba(197,159,89,0.3)"
                    stroke="#c59f59"
                    strokeWidth="2"
                  />
                </svg>
                <div className="absolute inset-0 flex flex-col justify-between items-center py-2 pointer-events-none">
                  <span className="text-[10px] font-bold text-heading uppercase surface-elevated px-2 py-0.5">
                    Technical
                  </span>
                  <span className="text-[10px] font-bold text-heading uppercase surface-elevated px-2 py-0.5">
                    Cultural
              ...
              <div className="grid grid-cols-2 gap-4 mt-6">
                <div>
                  ...
                  <p className="text-xl font-bold text-heading">92%</p>
                </div>
                <div>
                  <p className="text-[10px] text-subtle uppercase tracking-widest font-bold">
                    Problem Solving
                  </p>
                  <p className="text-xl font-bold text-heading">90%</p>
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="col-span-12 lg:col-span-8 flex flex-col gap-6 min-h-0 overflow-hidden">
            ...
            <div className="surface-card p-6 rounded">
              ...
              <p className="text-body text-sm leading-relaxed mb-6">
                Alex demonstrates an exceptional depth of knowledge in
                distributed systems and microservices architecture. His
                explanation of CAP theorem trade-offs was precise and reflected
                real-world implementation challenges. While technical
                proficiency is a clear outlier, he tended to dominate the
                conversation, occasionally missing subtle cues to let the
                interviewer interject.
              </p>
              ...
              <div className="flex-1 overflow-y-auto custom-scrollbar p-6 space-y-6 bg-slate-50/50 dark:bg-charcoal/30">
                {MOCK_TRANSCRIPT.map((entry) => (
                  <div key={entry.id} className="flex gap-4 relative group">
                    {entry.isKey && (
                      <div className="absolute -left-2 top-0 bottom-0 w-1 bg-primary scale-y-0 group-hover:scale-y-100 transition-transform" />
                    )}
                    <div className="w-12 text-[10px] font-bold text-primary/40 pt-1 shrink-0 uppercase tracking-tighter">
                      {entry.time}
                    </div>
                    <div className="space-y-2">
                      <div className="flex items-center justify-between">
                        <p
                          className={`text-[10px] font-bold uppercase tracking-widest ${entry.speaker === "ai" ? "text-primary" : "text-heading"}`}
                        >
